...
- show a form with fields of a DCA specified by you -> here the user can type in his data *(ATTENTION: Only ask for fields that are necessary for the action you're requesting the opt-in here)*
- this form can be prefilled using a prepared URL that can be generated by the insert tag `privacy_opt_in_url` (see InsertTag chapter)
...
- this action is only available with a prepared URL to a site containing the protocol entry module
- such a link can be created with the insert tag `privacy_opt_out_url` (see InsertTag chapter)
...
1. The information you type into the form is converted into an encrypted JWT token and appended to a prepared link (created with the insert tag `privacy_opt_in_url`)
...
---- | --------- | -------
privacy_opt_in_url | 1. The data for prefilling the form of `ModuleProtocolEntryEditor` and for the resulting privacy protocol entry in the format: `fieldName1:fieldValue1#fieldName2:fieldValue2`<br>2. The jumpTo page id<br>3. The data for finding a corresponding database entity to be linked to the protocol entry in the format: `table:referenceField:referenceFieldValue` (optional) | `{{privacy_opt_in_url::email:john@example.org#firstname:John#lastname:Doe::1::tl_member:email:john@example.org}}`
privacy_opt_out_url | 1. The data for the resulting privacy protocol entry in the format: `fieldName1:fieldValue1#fieldName2:fieldValue2`<br>2. The jumpTo page id<br>3. The data for finding a corresponding database entity to be linked to the protocol entry in the format: `table:referenceField:referenceFieldValue` (optional)<br>4. Set to `1` if the found entity also should be deleted if found (optional) | `{{privacy_opt_out_url::email:john@example.org#firstname:John#lastname:Doe::1::tl_member:email:john@example.org::1}}`
